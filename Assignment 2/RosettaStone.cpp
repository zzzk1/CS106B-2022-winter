...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    if (kGramLength <= 0) {
        error("The kGramLength can't be negative");
    }
    ...
    for (int i = 0; i <= str.length() - kGramLength; i++) {
        string unit = str.substr(i, kGramLength);
...
Map<string, double> normalize(const Map<string, double>& input) {
    if (input.size() == 0) {
        error("The input size must be positive.");
    }

    Map<string, double> result = {};
    double totalVal = 0;
    for (const string &key : input) {
        totalVal += pow(input.get(key), 2);
    }

    if (totalVal == 0) {
        error("The sum of input value can't be zero.");
    }
    totalVal = sqrt(totalVal);
    for (const string key : input) {
        result.put(key, input[key] / totalVal);
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    for (string key : source) {
        pq.enqueue(pair<string, double>{key, source.get(key)}, source.get(key));
    }
    ...
    Map<string, double> result = {};
    while (!pq.isEmpty()) {
        pair<string, double> p = pq.dequeue();
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    for (string key : lhs) {
        if (rhs.containsKey(key)) {
            double lhsValue = lhs.get(key);
            double rhsValue = rhs.get(key);
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for (Corpus cprpus : corpora) {
        double cosVal = cosineSimilarityOf(textProfile, cprpus.profile);
        pq.enqueue(cprpus.name, cosVal);
    }
